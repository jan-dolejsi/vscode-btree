...
suite('Workspace Test Suite', () => {
	...
	test('undeclared actions show up when manifest is created', async () => {
		...
		expect(treeWorkspace.getUndeclaredActions(tree1), "undeclared actions").to.be.empty;

		await treeWorkspace.initialization({
			action: () => treeWorkspace.addDeclaredAction('action1')
		});

		expect(treeWorkspace.getUndeclaredActions(tree1), "undeclared actions").to.include.members(['action2']);
...
function openTreeDocument(treeUri: vscode.Uri): void {
	const alreadyOpen = vscode.workspace.textDocuments.find(d => d.uri.toString() === treeUri.toString());
	if (alreadyOpen) {
		validateAndUpdateWorkspace(parser, alreadyOpen);
	}
	else {
		vscode.workspace.openTextDocument(treeUri);
	}
}